...
typography:
  display:
    fontFamily: "ABC Monument Grotesk, Inter, -apple-system, sans-serif"
    fontSize: 56px
    fontWeight: 700
    lineHeight: 1.0
...
**Orange-yellow as a "monetization" signal** — #FFBB00 communicates energy and financial optimism — yellow has long associations with gold and earnings. For a platform where revenue dashboards (ARR, paid subscribers) are core product surfaces, an accent that visually associates with money reinforces Beehiiv's core value proposition: your newsletter is a business.

**Dark marketing, light product** — The dark near-black marketing site (#0D0D0D) positions Beehiiv as ambitious and serious, differentiated from Substack's warmer editorial aesthetic. The light product dashboard then delivers on that seriousness with clean data density. The contrast between the two modes creates a "promise and delivery" narrative.

**ABC Monument Grotesk for headlines** — The heavyweight grotesque used on the marketing site communicates editorial authority and direct speech — appropriate for a platform targeting newsletter entrepreneurs who think of themselves as media operators, not hobbyist bloggers.

**Inter for product UI** — Switching to Inter in the dashboard reflects that the product is a professional tool, not a media brand. The functional neutrality of Inter keeps the subscriber tables, analytics charts, and editing tools from feeling overdesigned when the job is data analysis.

**Confetti on publish** — The success state animation on post publish is a deliberate behavioral reinforcement: the moment of hitting "publish" should feel like an achievement, encouraging the publishing cadence that drives subscriber growth (Beehiiv's core retention metric).
...
- **Primary on background** (#FFBB00 on #FFFFFF): 1.7:1 — decorative only
...
- **Focus indicator**: #FFBB00 outline, 2px, 2px offset
- **Focus contrast**: 1.7:1 against #FFFFFF background
...
- The yellow primary #FFBB00 has a contrast ratio of only 1.7:1 against white — it must never be used as text color or a standalone interactive indicator; it is decorative and structural (backgrounds, borders, icon fills) only.
- For focus rings, the yellow-on-white ratio is insufficient; supplement the yellow outline with a dark inner or outer shadow (e.g. `0 0 0 2px #0D0D0D`) to ensure the focus indicator itself is perceivable.
- Body text #0D0D0D at 19.4:1 and muted #6B7280 at 4.8:1 are both strong — the text system has no AA issues.
- Confetti and count-increment animations in the publish flow should be suppressed entirely under `prefers-reduced-motion`; provide a static success state as fallback.
